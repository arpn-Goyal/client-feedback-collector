...
export const getDashboardSummary = async (req, res) => {
  try {
    ...
    const summary = await FeedbackMongodb.aggregate([
      {
        $facet: {
          ...
          reactions: [
            {
              $match: { reaction: { $in: ["happy", "sad", "meh"] } },
            },
            {
              $group: {
                _id: "$reaction",
    ...
    const sad = reactions["sad"] || 0;
    const meh = reactions["meh"] || 0;

    const totalReactions = happy + sad + meh || 1;

    // 👍 Average sentiment score: from -1 (all sad) to +1 (all happy)
    const averageSentiment =
      totalReactions === 0 ? 0 : (happy - sad) / totalReactions;

    let sentimentLabel = "Neutral";
    if (averageSentiment >= 0.25) sentimentLabel = "Positive";
    else if (averageSentiment <= -0.25) sentimentLabel = "Negative";
    ...
    res.json({
      ...
      sentimentLabel, // 👈 added
      reactionStats: {
        happy,
        sad,
        meh,
        happyRatio: happy / totalReactions,
        sadRatio: sad / totalReactions,
        mehRatio: meh / totalReactions,
      },
    });
  } catch (error) {
